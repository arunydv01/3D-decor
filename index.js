...
function handleLoadRoom() {
    ...
    loader.load(
      ...
      (gltf) => {
        room = gltf.scene;
        applyTexture(room, textureURL, 0.1, 0.75); // Assuming applyTexture is defined elsewhere
        scene.add(room);
        loading.style.display = "none"; // Hide loading indicator
  
        // Function to change the color of the Plane mesh
        function changePlaneColor(mesh, color) {
          let randomColor = Math.random() * 0xffffff;
          let originalMaterial = mesh.material;
          let clonedMaterial = originalMaterial.clone();
          clonedMaterial.color.setHex(randomColor);
          mesh.material = clonedMaterial;
        }
  
        // Example usage to change Plane color to red (#ff0000)
        // changePlaneColor(room.getObjectByName("Plane"), 0xff0000);
  
        // Function to handle button click for changing Plane color
        const changeColorButton = document.getElementById("changeColorButton");
        changeColorButton.addEventListener("click", () => {
            // Change Plane color to random on button click
            changePlaneColor(room.getObjectByName("Plane"));
        });
      }
    );
  }
  
  // Function to apply texture to the room or mesh (assuming textureURL is defined elsewhere)
  function applyTexture(room, textureURL, scale, opacity) {
    // Implementation specific to your texture application
    // Replace with your actual texture application logic
  }
  
  // Initialize the scene once the DOM content is loaded
  document.addEventListener("DOMContentLoaded", () => {
    handleLoadRoom(); // Call handleLoadRoom function when DOM is ready
  });
